...
def drawline(lines, image):
    ...
    for line in lines:
        ...
        x2 = int(x0 - 1000 * (-b))
        y2 = int(y0 - 1000 * (a))
        cv2.line(copy, (x1, y1), (x2, y2), (0, 0, 255), 10)
...
def drawPoints(points, image):
    copy = image.copy()
    for point in points:
        cv2.circle(copy, tuple(map(int, point)), 30, (0, 255, 0), -1)
...
def process(image):
    ...
    cv2.imwrite("line.jpg", drawline(lines, image))
    ...
    cv2.imwrite("points.jpg", drawPoints(points, image))
    ...
    cv2.imwrite("quadrilateral.jpg", drawPoints(best, image))
